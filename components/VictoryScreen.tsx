...
export default function VictoryScreen({ comida, bebida, onRestart, onNewMenu }: VictoryScreenProps) {
  ...
  useEffect(() => {
    ...
    const isSad = lowerComida.includes('ensalada') || lowerComida.includes('agua') || lowerComida.includes('nada') || lowerBebida.includes('ensalada') || lowerBebida.includes('agua') || lowerBebida.includes('nada');

    const audio = new Audio(isSad ? '/audio/womp.mp3' : '/audio/tada.mp3');
    audio.play().catch(e => console.log('Audio error (may need interaction prior):', e));
  ...
  return (
    <div className="absolute inset-0 z-100 flex items-center justify-center bg-black/60 backdrop-blur-md overflow-hidden p-4">
      ...
      <div className={`transform transition-all duration-700 ease-out ${show ? 'scale-100 opacity-100 rotate-0' : 'scale-50 opacity-0 -rotate-12'} bg-white border-8 border-yellow-400 rounded-3xl p-6 md:p-10 max-w-2xl w-full text-center shadow-[0_0_100px_rgba(250,204,21,0.5)]`}>
        ...
        <div className="space-y-6 mb-10">
          <div className="bg-red-50 p-6 rounded-2xl border-4 border-red-200 shadow-inner">
            <p className="text-lg font-bold text-red-800 uppercase tracking-widest mb-1">Comerás</p>
            <p className="text-3xl md:text-4xl font-black text-red-600">{comida}</p>
          </div>
          
          <div className="bg-blue-50 p-6 rounded-2xl border-4 border-blue-200 shadow-inner">
            <p className="text-lg font-bold text-blue-800 uppercase tracking-widest mb-1">Acompañado de</p>
            <p className="text-3xl md:text-4xl font-black text-blue-600">{bebida}</p>
          </div>
        </div>
